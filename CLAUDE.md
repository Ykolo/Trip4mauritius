...
| 17 · Deux modes de vente — B1, le socle | ✅ schéma, disponibilité par chevauchement sous verrou, tarification au jour. **Sans UI publique : le tunnel touriste (B2) et la bascule des véhicules (B3) restent à faire** |

**139 tests verts** (`npm test`) : concurrence, RULE-001, annulation, cloisonnement opérateur, fuseau, modération, cascade des flags, catégories, cloisonnement des listings, écarts du catalogue admin, recherche par mot-clé, vocabulaire de filtrage, et cycle de vie des opérateurs.
...
- ⚠️ **L'envoi de photos est écrit mais pas provisionné.** `@vercel/blob` est installé, `/api/upload` émet le jeton signé (autorisation dans `onBeforeGenerateToken`, jamais côté client) et `ImageUploadButton` est branché dans l'éditeur de guide et le formulaire d'activité. **Il manque le store Blob et `BLOB_READ_WRITE_TOKEN`** — sur Vercel et dans le `.env` local. Tant qu'il manque, toute tentative d'envoi échoue et la saisie d'URL reste le chemin de repli. `upload()` ne transmet pas le message de la route : il jette « Failed to retrieve the client token » quelle que soit la cause. La raison lisible vient donc d'un `GET /api/upload`, que `ImageUploadButton` consulte **après** l'échec. Le logo opérateur, lui, est toujours en URL seule.
