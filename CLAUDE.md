...
- **Un calque flottant rendu sous un `sticky` ou un `transform` sort par un PORTAIL, pas par un `z-index` plus grand.** `position: sticky` crée un contexte d'empilement au même titre que `fixed`, **sans avoir besoin d'un `z-index`** — et `transform`, `filter`, `opacity < 1` font de même. Tout `z-index` posé en dessous n'est alors comparé qu'entre frères de ce sous-arbre : la confirmation « Ajouté au panier » portait `z-[70]` dans la colonne de réservation (`sticky top-24`), donc valait `0` face à la page, et **le logo du pied de page — qui vient après elle dans le DOM, à égalité — passait devant**. Le premier remède posé avait été de la remonter (`bottom-36`) pour esquiver les barres du bas : ça soignait le symptôme, jamais la cause. Le remède est `createPortal(…, document.body)` — `body` est le seul ancêtre dont on sait qu'aucun `sticky` ne viendra l'enfermer. Troisième cousin des deux règles ci-dessus, et il se mesure pareil : `document.elementsFromPoint(x, y)` au centre du recouvrement rend la pile de l'avant vers l'arrière ; si l'élément attendu n'est pas en tête, son `z-index` est mort. ⚠️ Un portail ne se crée pas au rendu serveur (`document` n'y existe pas) et `AnimatePresence` doit **survivre** au retrait de son enfant pour en animer la sortie : le portail est donc gardé par un drapeau `mounted`, jamais par `showToast &&`.
